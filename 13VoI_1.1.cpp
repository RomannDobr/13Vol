...
LRESULT CALLBACK PopupWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        ...
        if (g_inputValue == -1)
            vol = GetCurrentVolume99();
        else if (g_inputValue >= 0 && g_inputValue <= 9)
            vol = g_inputValue * 10;
        else if (g_inputValue >= 1000)
            // если ввод закончен (маркер > 1000), убираем маркер и отображаем реальное значение
            vol = (g_inputValue - 1000);
        else
            vol = g_inputValue;
        if (vol > 99)
            vol = 99;
...
LRESULT CALLBACK MsgWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_DIGIT_13VOI:
    {
        ...
        if (g_inputValue == -1)
            // первая цифра
            g_inputValue = digit;
        else if (g_inputValue <= 9)
        {
            // вторая цифра — формируем окончательное значение
            ...
            // После двух цифр ввод останавливается:
            // делаем невозможным добавление третьей цифры
            // (например, кодируем это специальным значением)
            g_inputValue = g_inputValue + 1000; // любое значение > 999 — просто маркер
        }
        else
            // если уже две цифры — игнорируем дальнейший ввод
            return 0;
    ...
    case WM_TIMER:
    {
        // если введена только одна цифра — применяем её * 10
        if (g_inputValue >= 0 && g_inputValue <= 9)
        {
